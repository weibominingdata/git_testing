...
if __name__ == "__main__":
    ...
    for line in open(fileB, 'r'):
        ...
        record = []
        posInfo = []
        chr, str, stp, type, a, b, c = line.split("\t")
        chromosome = "chr"+chr
        record.append(chromosome)
        posInfo.append(int(str))
        posInfo.append(int(stp))
        record.append(posInfo)
        record.append(int(type))
...
#    correctDetectedCNVBPS = 0    
        ...
        for y in contentB:
            if x[0]==y[0]:
                if aOverlapB(x[1],y[1]):
                    ...
                    if (y[2]>NORMAL and x[2]>NORMAL) or (y[2]<NORMAL and x[2]<NORMAL):
                        x[3] = x[3] + overlap
    ...
    nCNVsegInfer = 0
    nCNVbpsInfer = 0
    nCNVsegInferDetectedCorrect = 0
    nCNVsegInferDetected = 0
    nCNVbpsInferDetectedCorrect = 0
...
# add gold standard not in HMM output
    filegsnotHMMname = "%snotinHMM.dat"%(fileA)
    filegsnotHMM = open(filegsnotHMMname,'a')
    index = 0
    for y in contentB:
        index = index+1
        if y[0] == CHR:
            bps = y[1][1]-y[1][0]+1
            if (sum(y[3]) < bps*0.5):
               filegsnotHMM.write("%s\t%d\t%d\t%d\t%s\t%s\t%s\n"%(y[0],y[1][0],y[1][1],y[2],y[5],y[6],y[7]))
    filegsnotHMM.close()
...
#    filewholegenome.write("%s\t%f\t%d\t%d\t%f\t%d\t%d\t%f\t%d\t%d\t%f\t%d\t%d\t\n"%(fileC,float(nCNVdelcorrect)/nCNVdel, nCNVdel, nCNVdelcorrect, float(bpsCNVdelcorrect)/bpsCNVdel, bpsCNVdel, bpsCNVdelcorrect, float(nCNVsegCorrectInferDel)/nCNVsegInferDel, nCNVsegInferDel,nCNVsegCorrectInferDel, float(nCNVbpsCorrectInferDel)/nCNVbpsInferDel, nCNVbpsInferDel, nCNVbpsCorrectInferDel))
